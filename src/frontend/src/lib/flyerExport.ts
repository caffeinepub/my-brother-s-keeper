...
export async function exportFlyerAsPNG(
  ...
  try {
    ...
    if (!flyerElement) {
      throw new Error(
        "Flyer is not ready yet. Please wait a moment and try again.",
      );
    }
    ...
    if (!ctx) {
      throw new Error(
        "Your browser does not support this feature. Please try a different browser.",
      );
    }
    ...
    ctx.fillText(
      "Built with care for the trucking community",
      rect.width / 2,
      yOffset,
    );
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("taint") || error.message.includes("toBlob")) {
        throw new Error(
          'Unable to export flyer due to browser security restrictions. Please try again or use "Copy Link" instead.',
        );
      }
    ...
    throw new Error(
      'Failed to export flyer. Please try again or use "Copy Link" instead.',
    );
  }
}
