...
export function usePromoteToAdmin() {
  ...
  return useMutation<PromoteToAdminResult, Error, string>({
    ...
    onSuccess: (result) => {
      if (
        result.__kind__ === "success" ||
        result.__kind__ === "accountAlreadyAdmin"
      ) {
...
export function useCreateOrUpdateEmergencyProfile() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.createOrUpdateEmergencyProfile(
        nextOfKin,
        healthConditions,
        accessCode,
      );
...
export function useEmergencyLookup() {
  ...
  return useMutation({
    mutationFn: async ({
      user,
      accessCode,
    }: { user: Principal; accessCode: string }) => {
      if (!actor) throw new Error("Actor not available");
