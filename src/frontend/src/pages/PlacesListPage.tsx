    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Find trusted locations recommended by fellow truckers
          </p>
        </div>
        <AuthenticatedRouteGuard>
          <Button
            onClick={() => navigate({ to: "/places/add" })}
            className="gap-2"
          >
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <Select
          ...
          onValueChange={(value) =>
            setSelectedCategory(
              value === "all" ? null : (value as PlaceCategory),
            )
          }
        >
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              No places found. Be the first to add one!
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredPlaces.map((place) => (
            <Card
              ...
              onClick={() =>
                navigate({ to: `/places/${encodeURIComponent(place.name)}` })
              }
            >
              <CardHeader>
