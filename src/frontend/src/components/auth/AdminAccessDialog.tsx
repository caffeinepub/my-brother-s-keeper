...
export default function AdminAccessDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAuthenticated) {
      ...
      setToken("");
      toast.info(
        "Redirecting to login. Your admin token will be applied after authentication.",
      );
      await login();
      return;
    }
    ...
    promoteMutation.mutate(trimmedToken, {
      onSuccess: (result) => {
        if (result.__kind__ === "success") {
          ...
          // Keep dialog open for re-entry
          toast.error(
            "This admin token has expired. Please request a new one from an administrator.",
          );
        }
      },
      onError: (error) => {
        ...
        <DialogHeader>
          ...
          <DialogDescription asChild>
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>
                <strong className="text-foreground">Bootstrap admin:</strong> If
                you are the designated first administrator, simply log in with
                your registered principal — admin access is granted
                automatically, no token required.
              </p>
              <p>
                <strong className="text-foreground">Other users:</strong> Enter
                an admin token generated by an existing administrator to receive
                admin privileges.
              </p>
            </div>
          </DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!isAuthenticated && (
            <p className="text-sm text-muted-foreground bg-muted rounded-md p-3">
              <strong>Note:</strong> You are not logged in. Submitting will save
              your token and redirect you to login. Your admin privileges will
              be applied automatically after authentication.
            </p>
          )}

          {isAuthenticated && (
            <p className="text-sm text-muted-foreground bg-muted rounded-md p-3">
              You are logged in. Submitting will apply the token immediately to
              your account.
            </p>
          )}
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              type="submit"
              disabled={
                !token.trim() || promoteMutation.isPending || isLoggingIn
              }
            >
