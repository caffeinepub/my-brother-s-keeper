  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (licenseFile) {
        ...
        licenseBlob = ExternalBlob.fromBytes(licenseBytes).withUploadProgress(
          (percentage) => {
      ...
      if (idFile) {
        ...
        idBlob = ExternalBlob.fromBytes(idBytes).withUploadProgress(
          (percentage) => {
      ...
      await uploadVerification.mutateAsync({
        licenseProof: licenseBlob,
        idProof: idBlob,
      });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <Alert>
          <AlertDescription>
            Documents must be images (JPEG, PNG, WebP) or PDF files, maximum
            10MB each.
          </AlertDescription>
        </Alert>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            accept="image/*,.pdf"
            onChange={(e) =>
              handleFileChange("license", e.target.files?.[0] || null)
            }
