...
export default function SOSCardPage() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally omit lookupMutation.mutate
  useEffect(() => {
    if (userPrincipal && accessCode) {
      lookupMutation.mutate({ user: userPrincipal, accessCode });
    }
    // Only run when principal or accessCode changes, not on every render of lookupMutation
  }, [userPrincipal, accessCode]);
      ...
      <AuthenticatedRouteGuard>
        <div className="space-y-6 max-w-2xl mx-auto">
          ...
          <Alert>
            ...
            <AlertDescription>
              You need to set up your emergency profile first. Go to your
              profile page to create one.
            </AlertDescription>
          </Alert>
        </div>
      </AuthenticatedRouteGuard>
    ...
    <AuthenticatedRouteGuard>
      <div className="space-y-6 max-w-2xl mx-auto">
        <div>
          <h1 className="text-3xl font-bold">My Emergency Card</h1>
          <p className="text-muted-foreground">
            Preview of your emergency information
          </p>
        </div>
