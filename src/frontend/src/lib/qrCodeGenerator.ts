...
const EC_CODEWORDS_PER_BLOCK: Record<
  number,
  Record<ErrorCorrectionLevel, [number, number, number, number]>
> = {
  1: {
    L: [1, 19, 7, 0],
    M: [1, 16, 10, 0],
...
class QRCodeGenerator {
  ...
  private setupTypeInfo(maskPattern: number): void {
    const data =
      (ERROR_CORRECTION_LEVELS[this.errorCorrectionLevel] << 3) | maskPattern;
  ...
  private encodeData(data: string): number[] {
    ...
    // Get capacity info
    const ecInfo =
      EC_CODEWORDS_PER_BLOCK[this.version]?.[this.errorCorrectionLevel] ||
